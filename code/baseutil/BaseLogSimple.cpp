...
uint32_t CBaseLogSimple::LogInit(const char* pFile, bool bPrintf)
{
	// 检查参数
    if (pFile == nullptr)
    {
...
void CBaseLogSimple::LogInfo(const char* pSrcFile, const char* pFunc, int nLine, const char* fmt, ...)
{
    if (!m_cFileStm.is_open())
    {
        return;
...
void CBaseLogSimple::LogWarn(const char* pSrcFile, const char* pFunc, int nLine, const char* fmt, ...)
{
	if (!m_cFileStm.is_open())
	{
		return;
...
void CBaseLogSimple::LogEror(const char* pSrcFile, const char* pFunc, int nLine, const char* fmt, ...)
{
...
void CBaseLogSimple::WriteLog(int iLogLevel, const char* pSrcFile, const char* pFunc, int nLine)
{
    CheckLine();
    if (!m_cFileStm || !m_cFileStm.is_open())
    {
